...
def detach_torch(input):
    # if type is numpy, do not detach
...
class SPCI_and_EnbPI():
    ...
    def __init__(self, X_train, X_predict, Y_train, Y_predict, fit_func=None):
        ...
        self.criterion = 'squared_error' # 'mse' or 'mae'
    ...
    def one_boot_prediction(self, Xboot, Yboot, Xfull):
        if self.use_NeuralProphet:
            ...
            df_tmp, _ = make_NP_df(Xboot, Yboot)
    ...
    def fit_bootstrap_models_online_multistep(self, B, fit_sigmaX=True, stride=1):
        ...
        for s in range(stride):
            ...
            start = time.time()
            if self.use_NeuralProphet:
                self.df_full, self.Xnames = make_NP_df(
                    Xfull, np.zeros(n + n1))
            for b in range(B):
                self.b = b
    ...
    def get_results(self, alpha, data_name, itrial, true_Y_predict=[], method='Ensemble'):
        ...
        Ytest = detach_torch(self.Y_predict)
...
def test_EnbPI_or_SPCI(main_condition, results_EnbPI_SPCI, itrial=0):
    ...
    for train_frac in train_ls:
        ...
        Ytest = detach_torch(EnbPI.Y_predict)
...
def test_adaptive_CI(results_Adapt_CI, itrial=0):
    ...
    for train_frac in train_ls:
        ...
        for seed in seeds:
            ...
            if non_stat_solar:
                ...
                fit_func = RandomForestRegressor( n_estimators=5, quantiles=True, seed=seed) 
                #RangerForestRegressor(#n_estimators=5, quantiles=True, seed=seed)
            else:
                fit_func = RandomForestRegressor(n_estimators=10, quantiles=True, seed=seed)
    ...
    results_Adapt_CI.dict_full['AdaptiveCI'] = np.vstack(
        [cov_ls, width_ls])
    dict_to_latex(results_Adapt_CI.dict_full, train_ls)
